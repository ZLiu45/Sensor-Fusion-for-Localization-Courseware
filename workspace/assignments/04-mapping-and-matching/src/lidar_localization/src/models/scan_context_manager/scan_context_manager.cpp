...
ScanContextManager::ScanContextManager(const YAML::Node &node) {
  ...
  FAST_ALIGNMENT_SEARCH_RATIO_ =
      node["fast_alignment_search_ratio"].as<float>();
  // g. scan context distance threshold:
  SCAN_CONTEXT_DISTANCE_THRESH_ =
      node["scan_context_distance_thresh"].as<float>();
  ...
  LOG(INFO) << "Scan Context params:" << std::endl
            ...
            << MIN_KEY_FRAME_SEQ_DISTANCE_ << std::endl
            << "\tnearest-neighbor candidates to check: " << NUM_CANDIDATES_
            << std::endl
            << "\tfast alignment search ratio: " << FAST_ALIGNMENT_SEARCH_RATIO_
            << std::endl
            << "\tloop-closure scan context distance thresh: "
            << SCAN_CONTEXT_DISTANCE_THRESH_ << std::endl
            << std::endl;
...
std::pair<int, float> ScanContextManager::DetectLoopClosure(void) {
  ...
  if (0 == (state_.index_.counter_ % INDEXING_INTERVAL_)) {
 ...
 */
bool ScanContextManager::DetectLoopClosure(const CloudData &scan,
                                           Eigen::Matrix4f &pose) {
  // extract scan context and corresponding ring key:
  ...
  // get proposal:
  std::pair<int, float> proposal =
      GetLoopClosureMatch(query_scan_context, query_ring_key);
  ...
  Eigen::AngleAxisf orientation_change(yaw_change_in_rad,
                                       Eigen::Vector3f::UnitZ());
  pose.block<3, 3>(0, 0) =
      pose.block<3, 3>(0, 0) * orientation_change.toRotationMatrix();
...
bool ScanContextManager::Save(const std::string &output_path) {
  ...
  if (UpdateIndex(0)) {
    // prompt:
    LOG(INFO) << std::endl
              << "[Scan Context]: Index size "
              << state_.index_.kd_tree_->kdtree_get_point_count() << std::endl;

    // a. save index:
    std::string index_output_path = output_path + "/index.bin";
    if (!SaveIndex(index_output_path)) {
    ...
    // 1. scan contexts:
    std::string scan_contexts_output_path =
        output_path + "/scan_contexts.proto";
    if (!SaveScanContexts(scan_contexts_output_path)) {
      LOG(ERROR) << "[Scan Context]: Failed to write scan contexts."
                 << std::endl;
      return false;
    } else {
      LOG(INFO) << "\tSave scan context of size " << state_.scan_context_.size()
                << " to: " << scan_contexts_output_path << std::endl;
    }

    // 2. ring keys:
    std::string ring_keys_output_path = output_path + "/ring_keys.proto";
    if (!SaveRingKeys(ring_keys_output_path)) {
      LOG(ERROR) << "[Scan Context]: Failed to write ring keys." << std::endl;
      return false;
    } else {
      LOG(INFO) << "\tSave ring keys of size "
                << state_.index_.data_.ring_key_.size()
                << " to: " << ring_keys_output_path << std::endl;
    }

    // 3. key frames:
    std::string key_frames_output_path = output_path + "/key_frames.proto";
    if (!SaveKeyFrames(key_frames_output_path)) {
      LOG(ERROR) << "[Scan Context]: Failed to write key frames." << std::endl;
      return false;
    } else {
      LOG(INFO) << "\tSave key frames of size "
                << state_.index_.data_.key_frame_.size()
                << " to: " << key_frames_output_path << std::endl;
    }

    google::protobuf::ShutdownProtobufLibrary();
  } else {
    LOG(ERROR) << std::endl
               << "[Scan Context]: Skip empty index" << std::endl
               << std::endl;
  }
...
bool ScanContextManager::Load(const std::string &input_path) {
  ...
  LOG(INFO) << std::endl
            << "[Scan Context]: Load Scan Context Index." << std::endl;

  // 1. scan contexts:
  std::string scan_contexts_input_path = input_path + "/scan_contexts.proto";
  ;
  if (!LoadScanContexts(scan_contexts_input_path)) {
  ...
  if (!LoadRingKeys(ring_keys_input_path)) {
  ...
  if (!LoadKeyFrames(key_frames_input_path)) {
  ...
  state_.index_.kd_tree_ = std::make_shared<RingKeyIndex>(
      NUM_RINGS_,                       /* dim */
      state_.index_.data_.ring_key_, 10 /* max leaf size */
  );
 ...
 */
ScanContextManager::ScanContext
ScanContextManager::GetScanContext(const CloudData &scan) {
  ...
  ScanContext scan_context =
      UNKNOWN_HEIGHT * ScanContext::Ones(NUM_RINGS_, NUM_SECTORS_);
 ...
 */
ScanContextManager::RingKey
ScanContextManager::GetRingKey(const ScanContext &scan_context) {
...
void ScanContextManager::GenerateRandomRingKey(
    ScanContextManager::RingKeys &samples, const int N, const int D,
 ...
 */
Eigen::MatrixXf
ScanContextManager::GetSectorKey(const Eigen::MatrixXf &scan_context) {
 ...
 */
float ScanContextManager::GetOrientation(const float &x, const float &y) {
 ...
 */
int ScanContextManager::GetIndex(const float &value, const float &MAX_VALUE,
 ...
 * @return void
 */
void ScanContextManager::GetCandidateIndices(const RingKey &ring_key,
                                             const int N,
                                             std::vector<size_t> &indices,
                                             std::vector<float> &distances) {
  state_.index_.kd_tree_->query(&ring_key.at(0), N, &indices.at(0),
                                &distances.at(0));
}
 ...
 */
Eigen::MatrixXf ScanContextManager::CircularShift(const Eigen::MatrixXf &mat,
                                                  int shift) {
  if (0 == shift) {
  ...
  Eigen::MatrixXf shifted_mat = Eigen::MatrixXf::Zero(mat.rows(), mat.cols());
...
int ScanContextManager::GetOptimalShiftUsingSectorKey(
    const Eigen::MatrixXf &target, const Eigen::MatrixXf &source) {
  int optimal_shift = 0;
  float optimal_dist = std::numeric_limits<float>::max();

  for (int curr_shift = 0; curr_shift < target.cols(); ++curr_shift) {
    Eigen::MatrixXf curr_target = CircularShift(target, curr_shift);
...
float ScanContextManager::GetCosineDistance(
  ...
  for (int sid = 0; sid < N; ++sid) {
    ...
    float sector_similarity = target_sector.dot(source_sector) /
                              (target_sector_norm * source_sector_norm);

    sum_sector_similarity += sector_similarity;
    ++num_effective_cols;
  }

  return (0 == num_effective_cols
              ? 1.0f
              : (1.0f - sum_sector_similarity / num_effective_cols));
}
...
std::pair<int, float> ScanContextManager::GetScanContextMatch(
  ...
  int sector_key_shift =
      GetOptimalShiftUsingSectorKey(target_sector_key, source_sector_key);

  // generate precise alignment proposals:
  const int N = target_scan_context.cols();
  const int SEARCH_RADIUS = round(0.5 * FAST_ALIGNMENT_SEARCH_RATIO_ * N);
  std::vector<int> candidate_shifts{sector_key_shift};
  for (int r = 1; r < SEARCH_RADIUS + 1; ++r) {
    candidate_shifts.push_back((sector_key_shift + r + N) % N);
    candidate_shifts.push_back((sector_key_shift - r + N) % N);
  }
  ...
  for (int curr_shift : candidate_shifts) {
    ScanContext target_scan_context_shifted =
        CircularShift(target_scan_context, curr_shift);

    float curr_dist =
        GetCosineDistance(target_scan_context_shifted, source_scan_context);
...
bool ScanContextManager::UpdateIndex(const int MIN_KEY_FRAME_SEQ_DISTANCE) {
  if (state_.ring_key_.size() >
      static_cast<size_t>(MIN_KEY_FRAME_SEQ_DISTANCE)) {
    // fetch to-be-indexed ring keys from buffer:
    state_.index_.data_.ring_key_.clear();
    state_.index_.data_.ring_key_.insert(
        state_.index_.data_.ring_key_.end(), state_.ring_key_.begin(),
    ...
    state_.index_.data_.key_frame_.insert(
        state_.index_.data_.key_frame_.end(), state_.key_frame_.begin(),
        // this ensures the min. key frame seq. distance
 ...
 */
std::pair<int, float>
ScanContextManager::GetLoopClosureMatch(const ScanContext &query_scan_context,
                                        const RingKey &query_ring_key) {
  int match_id = NONE;

  //
  // step 1: loop closure detection criteria check -- only perform loop closure
  // detection when
  //   a. current key scan is temporally far away from previous key scan:
  //
  if (state_.ring_key_.size() <=
      (static_cast<size_t>(MIN_KEY_FRAME_SEQ_DISTANCE_))) {
  ...
  GetCandidateIndices(query_ring_key, NUM_CANDIDATES_, candidate_indices,
                      candidate_distances);
  ...
  for (int i = 0; i < NUM_CANDIDATES_; ++i) {
    const ScanContext &candidate_scan_context =
        state_.scan_context_.at(candidate_indices.at(i));

    std::pair<int, float> match_result =
        GetScanContextMatch(candidate_scan_context, query_scan_context);
  ...
  if (optimal_dist < SCAN_CONTEXT_DISTANCE_THRESH_) {
    ...
    LOG(INFO) << std::endl
              ...
              << state_.scan_context_.size() - 1 << "<-->" << optimal_index
              << std::endl
              << "\tDistance " << optimal_dist << std::endl
              << "\tHeading Change " << yaw_change_in_deg << " deg."
              << std::endl
              << std::endl;
  }
...
bool ScanContextManager::SaveScanContexts(const std::string &output_path) {
  ...
  for (size_t i = 0; i < state_.scan_context_.size(); ++i) {
    ...
    scan_context_io::ScanContext *output_scan_context =
        scan_contexts.add_data();

    for (int rid = 0; rid < NUM_RINGS_; ++rid) {
      for (int sid = 0; sid < NUM_SECTORS_; ++sid) {
        output_scan_context->add_data(input_scan_context(rid, sid));
      }
    }
  }

  std::fstream output(output_path,
...
bool ScanContextManager::LoadScanContexts(const std::string &input_path) {
  ...
  for (int i = 0; i < scan_contexts.data_size(); ++i) {
    const scan_context_io::ScanContext &input_scan_context =
        scan_contexts.data(i);
...
bool ScanContextManager::LoadKeyFrames(const std::string &input_path) {
  ...
  for (int i = 0; i < key_frames.data_size(); ++i) {
    ...
    Eigen::Quaternionf input_q(input_key_frame.q().w(), input_key_frame.q().x(),
                               input_key_frame.q().y(),
                               input_key_frame.q().z());
    Eigen::Vector3f input_t(input_key_frame.t().x(), input_key_frame.t().y(),
                            input_key_frame.t().z());
