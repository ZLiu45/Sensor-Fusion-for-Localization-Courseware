...
bool GNSSINSSimFiltering::Correct(const IMUData &imu_data,
  ...
  current_measurement_.T_nb(0, 3) =
      static_cast<double>(pos_vel_mag_data.pos.x());
  current_measurement_.T_nb(1, 3) =
      static_cast<double>(pos_vel_mag_data.pos.y());
  current_measurement_.T_nb(2, 3) =
      static_cast<double>(pos_vel_mag_data.pos.z());
  ...
  if (kalman_filter_ptr_->Correct(imu_data, CONFIG.FUSION_STRATEGY,
    ...
    if (0 == (++count % 10)) {
      ...
      kalman_filter_ptr_->UpdateObservabilityAnalysis(pos_vel_mag_data.time,
...
void GNSSINSSimFiltering::SaveObservabilityAnalysis(void) {
  kalman_filter_ptr_->SaveObservabilityAnalysis(CONFIG.FUSION_STRATEGY);
}
...
bool GNSSINSSimFiltering::InitFusion(const YAML::Node &config_node) {
  ...
  LOG(INFO) << "fusion strategy: " << fusion_strategy;

  if (CONFIG.FUSION_STRATEGY_ID.end() !=
      CONFIG.FUSION_STRATEGY_ID.find(fusion_strategy)) {
