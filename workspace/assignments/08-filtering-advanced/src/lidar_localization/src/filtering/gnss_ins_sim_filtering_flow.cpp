...
GNSSINSSimFilteringFlow::GNSSINSSimFilteringFlow(ros::NodeHandle &nh) {
  ...
  pos_vel_mag_sub_ptr_ =
      std::make_shared<PosVelMagSubscriber>(nh, "/synced_pos_vel_mag", 100000);
...
bool GNSSINSSimFilteringFlow::Run() {
  ...
  while (HasData()) {
    if (!HasInited()) {
      if (HasPosVelMagData() && ValidPosVelMagData() && HasIMUData() &&
          ValidIMUData()) {
        InitLocalization();
      }
    } else {
      ...
      if (HasPosVelMagData() && ValidPosVelMagData()) {
        if (HasIMUData()) {
          ...
          if (current_imu_data_.time >= current_pos_vel_mag_data_.time) {
...
bool GNSSINSSimFilteringFlow::SaveOdometry(void) {
  if (0 == trajectory.N || has_recorded_) {
    return false;
  }
  ...
  if (!FileManager::CreateFile(fused_odom_ofs,
      ...
      !FileManager::CreateFile(
          gnss_odom_ofs, WORK_SPACE_PATH + "/slam_data/trajectory/gnss.txt") ||
...
bool GNSSINSSimFilteringFlow::InitLocalization(void) {
  ...
  Eigen::Vector3f init_vel =
      current_imu_data_.GetOrientationMatrix() * current_pos_vel_mag_data_.vel;
...
bool GNSSINSSimFilteringFlow::CorrectLocalization() {
  ...
  if (
      ...
      0 == (++count % 40) &&
      // successful correct:
      filtering_ptr_->Correct(current_imu_data_, current_pos_vel_mag_data_)) {
