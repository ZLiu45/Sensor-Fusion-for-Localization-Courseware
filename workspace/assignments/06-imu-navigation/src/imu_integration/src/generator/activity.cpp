...
void Activity::Init(void) {
  // parse IMU config:
  private_nh_.param("imu/device_name", imu_config_.device_name,
  ...
  private_nh_.param("imu/gyro/sigma_noise", imu_config_.gyro_noise_stddev,
                    0.015);
  ...
  private_nh_.param("pose/frame_id", odom_config_.frame_id,
                    std::string("inertial"));
  private_nh_.param("pose/topic_name", odom_config_.topic_name.ground_truth,
...
void Activity::GetGroundTruth(void) {
  ...
  Eigen::Vector3d p(kRhoX * cos_w_xy_t, kRhoY * sin_w_xy_t, kRhoZ * sin_w_z_t);
  Eigen::Vector3d v(-rho_x_w_xy * sin_w_xy_t, rho_y_w_xy * cos_w_xy_t,
                    rho_z_w_z * cos_w_z_t);
  Eigen::Vector3d a(-rho_x_w_xy * kOmegaXY * cos_w_xy_t,
  ...
  Eigen::Vector3d euler_angles(kRoll * cos_t, kPitch * sin_t,
...
void Activity::AddNoise(double delta_t) {
  ...
  Eigen::Vector3d angular_vel_noise =
      GetGaussianNoise(imu_config_.gyro_noise_stddev / sqrt_delta_t);
  Eigen::Vector3d linear_acc_noise =
...
void Activity::SetOdometryMessage(void) {
  ...
  // d. set velocity:    void WritePoseToFile();
...
Eigen::Matrix3d
Activity::EulerAnglesToRotation(const Eigen::Vector3d &euler_angles) {
  // parse Euler angles:
...
void Activity::WritePoseToFile() {
  if (output_stream_.is_open()) {
    output_stream_ << R_gt_(0, 0) << " " << R_gt_(0, 1) << " " << R_gt_(0, 2)
                   << " " << t_gt_(0) << " " << R_gt_(1, 0) << " "
