...
ScanMapRegistration::ScanMapRegistration(void) {
  std::string config_file_path =
      WORK_SPACE_PATH + "/config/front_end/loam.yaml";
...
bool ScanMapRegistration::Update(const CloudDataXYZI::CLOUD_PTR sharp_points,
  ...
  if (HasSufficientFeaturePoints(local_map)) {
    ...
    for (int i = 0; i < config_.max_num_iteration; ++i) {
      // build problem:
      CeresALOAMRegistration aloam_registration(config_.registration_config,
                                                pose_.scan_map_odometry.q,
                                                pose_.scan_map_odometry.t);
      AddEdgeFactors(filtered_sharp_points, local_map.sharp,
                     aloam_registration);
      AddPlaneFactors(filtered_flat_points, local_map.flat, aloam_registration);

      // get relative pose:
      aloam_registration.Optimize();
      aloam_registration.GetOptimizedRelativePose(pose_.scan_map_odometry.q,
                                                  pose_.scan_map_odometry.t);
    }
  }
  ...
  submap_ptr_->RegisterLineFeaturePoints(filtered_sharp_points,
                                         pose_.scan_map_odometry.q,
                                         pose_.scan_map_odometry.t);
  submap_ptr_->RegisterPlaneFeaturePoints(filtered_flat_points,
                                          pose_.scan_map_odometry.q,
                                          pose_.scan_map_odometry.t);
  ...
  if (0 == ++dowmsample_count % 1) {
    submap_ptr_->DownsampleSubMap(filter_.sharp_filter_ptr_,
                                  filter_.flat_filter_ptr_);
...
bool ScanMapRegistration::InitFilters(const YAML::Node &config_node) {
  filter_.sharp_filter_ptr_ =
      std::make_unique<VoxelFilter>(config_node["sharp"]);
...
bool ScanMapRegistration::HasSufficientFeaturePoints(
    const aloam::SubMap::LocalMap &local_map) {
  const auto num_sharp_points =
      static_cast<int>(local_map.sharp->points.size());
  const auto num_flat_points = static_cast<int>(local_map.flat->points.size());

  return ((num_sharp_points > config_.min_num_sharp_points) &&
          (num_flat_points > config_.min_num_flat_points));
}

bool ScanMapRegistration::SetTargetPoints(aloam::SubMap::LocalMap &local_map) {
...
bool ScanMapRegistration::ProjectToMapFrame(
    const CloudDataXYZI::CLOUD_PTR &source, CloudDataXYZI::CLOUD_PTR &query) {
  Eigen::Matrix4f transform_matrix = Eigen::Matrix4f::Identity();

  transform_matrix.block<3, 3>(0, 0) =
      pose_.scan_map_odometry.q.toRotationMatrix().cast<float>();
...
int ScanMapRegistration::AddEdgeFactors(
  ...
  for (size_t i = 0; i < num_feature_points; ++i) {
    ...
    kdtree_.sharp->nearestKSearch(feature_point_in_map_frame, num_neighbors,
                                  target_candidate_indices,
                                  target_candidate_distances);
    if (target_candidate_distances.back() < config_.distance_thresh) {
      ...
      for (size_t i = 0; i < num_neighbors; ++i) {
        ...
        Eigen::Vector3d x{target_point.x, target_point.y, target_point.z};
      ...
      if (saes.eigenvalues()[2] > 3 * saes.eigenvalues()[1]) {
        ...
        const Eigen::Vector3d source{feature_point_in_lidar_frame.x,
        ...
        aloam_registration.AddEdgeFactor(source, target_x, target_y, 1.0);
  ...
  for (size_t i = 0; i < num_feature_points; ++i) {
    ...
    if (target_candidate_distances.back() < config_.distance_thresh) {
      ...
      if (residual.maxCoeff() <= 0.2) {
        ...
        // use predicted odometry from scan-scan odometry as initial value:
        aloam_registration.AddPlaneNormFactor(source, norm,
...
bool ScanMapRegistration::PredictScanMapOdometry(
    const Eigen::Matrix4f &odom_scan_to_scan) {
  // set scan-scan estimation, ALOAM original implementation:
  // pose_.scan_scan_odometry.q = odom_scan_to_scan.block<3, 3>(0,
  // 0).cast<double>(); pose_.scan_scan_odometry.t = odom_scan_to_scan.block<3,
  // 1>(0, 3).cast<double>();
...
bool ScanMapRegistration::UpdateRelativePose(void) {
  ...
  pose_.relative.q =
      pose_.scan_map_odometry.q * pose_.scan_scan_odometry.q.inverse();
  pose_.relative.t =
      pose_.scan_map_odometry.t - pose_.relative.q * pose_.scan_scan_odometry.t;

  return true;
}

bool ScanMapRegistration::UpdateOdometry(Eigen::Matrix4f &lidar_odometry) {
  lidar_odometry.block<3, 3>(0, 0) =
      pose_.scan_map_odometry.q.toRotationMatrix().cast<float>();
