...
bool SlidingWindowFlow::ValidData() {
  ...
  double diff_map_matching_odom_time =
      current_laser_odom_data_.time - current_map_matching_odom_data_.time;
  double diff_imu_time = current_laser_odom_data_.time - current_imu_data_.time;
  double diff_gnss_pose_time =
      current_laser_odom_data_.time - current_gnss_pose_data_.time;

  if (diff_map_matching_odom_time < -0.05 || diff_imu_time < -0.05 ||
      diff_gnss_pose_time < -0.05) {
