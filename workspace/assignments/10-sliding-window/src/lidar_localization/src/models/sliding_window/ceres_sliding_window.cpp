...
void CeresSlidingWindow::AddPRVAGRelativePoseFactor(
  ...
  // b.1. position:
  residual_relative_pose.m.block<3, 1>(INDEX_P, 0) =
      relative_pose.block<3, 1>(0, 3);
  // b.2. orientation, so3:
  residual_relative_pose.m.block<3, 1>(INDEX_R, 0) =
      Sophus::SO3d(
...
void CeresSlidingWindow::AddPRVAGMapMatchingPoseFactor(
    const int param_index, const Eigen::Matrix4d &prior_pose,
  ...
  // b.1. position:
  residual_map_matching_pose.m.block<3, 1>(INDEX_P, 0) =
      prior_pose.block<3, 1>(0, 3);
  // b.2. orientation, so3:
  residual_map_matching_pose.m.block<3, 1>(INDEX_R, 0) =
      Sophus::SO3d(
  ...
  if ((kWindowSize + 1 <= N)) {
    ...
    if (!residual_blocks_.map_matching_pose.empty()) {
      for (const auto &residual_map_matching_pose :
        ...
        problem.AddResidualBlock(factor_map_matching_pose, NULL,
                                 key_frame.prvag);
      }
    }
    ...
    if (!residual_blocks_.relative_pose.empty()) {
      for (const auto &residual_relative_pose :
        ...
        problem.AddResidualBlock(factor_relative_pose, NULL, key_frame_i.prvag,
                                 key_frame_j.prvag);
      }
    }
    ...
    if (!residual_blocks_.imu_pre_integration.empty()) {
      for (const auto &residual_imu_pre_integration :
        ...
        problem.AddResidualBlock(factor_imu_pre_integration, NULL,
...
bool CeresSlidingWindow::GetLatestOptimizedKeyFrame(
  ...
  optimized_key_frame = KeyFrame(N - 1, latest_optimized_key_frame.time,
                                 latest_optimized_key_frame.prvag);
...
bool CeresSlidingWindow::GetOptimizedKeyFrames(
  ...
  for (int param_id = 0; param_id < N; param_id++) {
    ...
    optimized_key_frames.emplace_back(param_id, optimized_key_frame.time,
                                      optimized_key_frame.prvag);
  }
 ...
 */
Eigen::MatrixXd
CeresSlidingWindow::GetInformationMatrix(Eigen::VectorXd noise) {
